    ...
    <div class="main">
      <el-card class="box-card" v-for="(item,index) in tableData" :key="index">
        ...
        <div v-if="isexpand[index]">
          <el-input placeholder="请输入内容" v-model="mainInput" clearable></el-input>
          <br />
          <el-button
            type="success"
            size="mini"
        ...
        <div :style="{'margin-top':isexpand[index]==true?'50px':'0px'}">
          <div class="div1" v-for="(item2,index2) in item.mainComment" :key="index2">
            <div class="user-face">
              <img src="./i1.jpg" alt />
            </div>
            <div class="con" :style="{'margin-top':isexpand[index]==true?'20px':'0px'}">
              <div class="user">
              ...
              <div class="info">
                ...
                <span class="reply" @click="expandCommentItem2(item2.uid,item2.id)">
                  <el-link icon="el-icon-edit">回复</el-link>
                </span>
              </div>

              <div class="reply-box">
                <div class="reply-item" v-for="(item3,index3) in item2.sideComment" :key="index3">
                  ...
                  <div class="reply-con">
                    <div class="user">
                      <span style="font-size:14px;margin-right:5px;">{{item3.uid}}</span>
                      <span style="margin-right:5px;" v-if="item3.replyTo!=''">@{{item3.replyTo}}</span>
                      :
                      <span
                        class="text"
                  ...
                  <div class="info">
                    ...
                    <span class="reply" @click="expandCommentItem(item3.uid,item3.id)">
                      <el-link icon="el-icon-edit">回复</el-link>
                    </span>
                  </div>

                  <div v-if="replyIsexpand[item3.id]" style="margin-top:10px;margin-bottom:45px">
                    <el-input :placeholder="'@'+replyTo[item3.id]" v-model="sideInput" clearable></el-input>
              ...
              <div v-if="reply_main[item2.id]" style="margin-top:10px;margin-bottom:45px">
                <el-input :placeholder="'@'+replyTo[item2.id]" v-model="input1" clearable></el-input>
                <br />
                <el-button
                  type="success"
                  size="mini"
  ...
  methods: {
    ...
    sendMainComment(id) {
      ...
      senMainCommentApi(mainCommentList)
        ...
        // let opt = {
        //   method: "post",
        //   body: JSON.stringify(mainCommentList),
        //   headers: {
