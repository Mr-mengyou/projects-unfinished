<template>
  <el-main>
    <el-table :data="tableData" style="width: 100%">
      ...
      ></el-table-column>
      <el-table-column
        prop="groupUploadPath"
        label="报名材料"
        width="200"
        align="center"
      >
        <template slot-scope="scope">
          <a :href="scope.row.groupUploadPath">{{
            scope.row.groupUploadPath.substring(
              scope.row.groupUploadPath.lastIndexOf('/') + 1
            )
          }}</a>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center">
        <template slot-scope="scope">
          <!-- <el-button
            :disabled="scope.row.status == '报名已结束'"
            v-if="scope.row.playerName.split('：')[0] == '小组组长'"
            size="mini"
...
<script>
import { recordApi } from '../../request/studentApi'
import { editPersonApi } from '../../request/studentApi'
import { deletePersonActivApi } from '../../request/studentApi'
export default {
  data() {
    return {
      isShow: false,
  ...
  methods: {
    getRecord() {
      let personUid = decodeURIComponent(
    ...
    cancelApply(currentRow) {
      let cancelLevel = currentRow.playerName.split('：')[0]
      if (cancelLevel == '小组组长') {
        this.$confirm(
          '因为你是参赛小组组长,您的删除操作,就意味参赛小组解散?',
          '提示',
          {
            confirmButtonText: '确定解散',
            cancelButtonText: '取消解散',
            type: 'warning',
          }
        )
          .then(() => {
            ...
            let matchName = currentRow.matchName
            let level = 1
            deletePersonActivApi(personUid, matchName, level).then((res) => {
              if (res.statusCode == 1) {
                this.$message({
                  type: 'success',
                  message: '解散成功',
    ...
    editPerson(currentRow) {
      ...
      editPersonApi(currentRow.playerId, currentRow.matchName).then((res) => {
        for (let i = 0; i < res.data.length; i++) {
          this.ruleForm.crew[i].stuno = res.data[i].playerId
          this.ruleForm.crew[i].stuname = res.data[i].playerName
        }
        this.isShow = true
    ...
    uploadSuccess(response, file, fileList) {
      ...
      if (response.code == 0) {
        this.ruleForm.originId = decodeURIComponent(
          window.atob(sessionStorage.getItem('_loginId'))
        )
